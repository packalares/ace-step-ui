"""PATCHED — re-prioritize raw_lyrics over the LM's own transcription path.

Diff vs upstream `acestep/training/dataset_builder_modules/label_single.py`:

When auto-label is invoked with `transcribe_lyrics=True` AND the sample
already has `raw_lyrics` populated (e.g. from companion `.txt` files
written by our `whisper_cli.py` during stem extraction), upstream goes:

    if sample.is_instrumental: ...
    elif transcribe_lyrics:
        sample.lyrics = llm_lyrics       # <-- LM tries to transcribe; bad
                                         #     for any non-EN/ZH language
    elif has_preloaded_lyrics:
        sample.lyrics = sample.raw_lyrics  # <-- never reached

So the LM's bad transcription overrides Whisper's good one.

Fix: when `has_preloaded_lyrics` is True (i.e. we already have a
high-quality multilingual Whisper transcript on disk), skip the LM's
lyric-transcription path and use the preloaded text directly. The LM
still does caption / genre / BPM detection — only the lyric extraction
gets short-circuited.

This is the only patch we need. The rest of label_single.py is stock.
"""
...
class LabelSingleMixin:
    ...
    def label_sample(
        ...
        # If we have preloaded raw lyrics (e.g. from whisper_cli companion
        # .txt files written during stem extraction), don't ask the LM to
        # re-transcribe — its multilingual transcription is poor compared
        # to Whisper-large-v3.
        ...
        try:
            ...
            if format_lyrics and has_preloaded_lyrics:
                ...
                if sample.is_instrumental:
                    ...
                    # PATCH: prioritize preloaded lyrics (e.g. from Whisper
                    # companion files) over the LM's lyric output. This branch
                    # used to come AFTER `transcribe_lyrics`; we put it first
                    # by short-circuiting transcribe_lyrics above.
                    sample.lyrics = sample.raw_lyrics
                    sample.formatted_lyrics = ""
                    # Use the LM's vocal_language guess only as a last-resort fallback.
                    sample.language = sample.language or metadata.get("vocal_language", "unknown")
                    status_suffix = "(using preloaded lyrics)"
                elif transcribe_lyrics:
                    sample.language = metadata.get("vocal_language", "unknown")
                    sample.formatted_lyrics = metadata.get("lyrics", "")
