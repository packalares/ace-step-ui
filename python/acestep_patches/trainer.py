        ...
        for epoch in range(start_epoch, self.training_config.max_epochs):
            ...
            for _batch_idx, batch in enumerate(train_loader):
                ...
                if training_state and training_state.get("should_stop", False):
                    # Save current weights to <output_dir>/final/adapter so the
                    # user doesn't lose in-flight progress when they hit Stop.
                    # Without this, every Stop = full retrain from scratch.
                    try:
                        _stop_final_path = os.path.join(self.training_config.output_dir, "final")
                        save_lora_weights(self.module.model, _stop_final_path)
                        logger.info(f"⏹️ Stop requested — saved current weights to {_stop_final_path}")
                        _stop_msg = f"⏹️ Training stopped by user — saved to {_stop_final_path}"
                    except Exception:
                        logger.exception("Failed to save adapter on stop")
                        _stop_msg = "⏹️ Training stopped by user (save failed; check logs)"
                    yield (
                        global_step,
            ...
            for batch in train_loader:
                if training_state and training_state.get("should_stop", False):
                    ...
                    try:
                        ...
                        save_lora_weights(self.module.model, _stop_final_path)
                        logger.info(f"⏹️ Stop requested — saved current weights to {_stop_final_path}")
                        _stop_msg = f"⏹️ Training stopped — saved to {_stop_final_path}"
                    except Exception:
                        logger.exception("Failed to save adapter on stop")
                        _stop_msg = "⏹️ Training stopped (save failed; check logs)"
                    yield (
                        global_step,
                        accumulated_loss / max(accumulation_step, 1),
                        _stop_msg,
